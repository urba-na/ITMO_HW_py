...
def print_details(market, avg_rating, reviews):
    ...
    print(f"{market[4] if market[4] else 'N/A'} | ZIP: {market[4] if market[4] else 'N/A'}")
    print(f"Координаты: {market[5] if market[5] else 'N/A'}/{market[6] if market[6] else 'N/A'}")
    print(f"Время обновления: {market[7] if market[7] else 'N/A'}")  
    print(f"Товары: {market[8] if market[8] else 'нет данных'}")        
    print(f"Сервисы: {market[9] if market[9] else 'нет данных'}")    
...
def print_intro_del():
    print('Выбранный рынок будет удален из базы данных навсегда!!!')
    print('Введите fmid рынка')
