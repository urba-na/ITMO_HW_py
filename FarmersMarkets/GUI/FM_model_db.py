...
def markets_list(limit, offset=0):
    ...
    cur.execute("""
        SELECT 
            m.FMID,
            m.MarketName,
            COALESCE(cn.CountyName, 'N/A') as CountyName,
...
def get_market_details(fmid):
    cur.execute("""
        SELECT 
            m.fmid,
            m.marketname,
            COALESCE(a.street, m.location) as street,
            COALESCE(c.countyname, 'N/A') as county,
            a.zip,
            a.x,
            a.y,
            m.updatetime,
            COALESCE(STRING_AGG(DISTINCT p.productname, ', '), 'нет данных') as products,
            COALESCE(STRING_AGG(DISTINCT s.servicename, ', '), 'нет данных') as services
        FROM markets m
        LEFT JOIN addresses a ON m.fmid = a.fmid
        LEFT JOIN counties c ON a.countyid = c.countyid
        LEFT JOIN market_products mp ON m.fmid = mp.fmid
        LEFT JOIN products p ON mp.productid = p.productid
        LEFT JOIN market_services ms ON m.fmid = ms.fmid
        LEFT JOIN services s ON ms.serviceid = s.serviceid
        WHERE m.fmid = %s
        GROUP BY m.fmid, m.marketname, a.street, m.location, c.countyname, 
                 a.zip, a.x, a.y, m.updatetime
    """, (fmid,))
    return cur.fetchone()
...
def get_markets_zip_radius(limit=2000, offset=0):
    cur.execute("""
        SELECT 
            m.fmid,
            m.marketname,
